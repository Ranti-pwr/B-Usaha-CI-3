...
class Model_Keuangan extends CI_Model
{
	...
	{
		$bulan = date('n');
		$this->db->select('SUM(total_untung) as total_bulan');
		$this->db->from('penjualan');
		$this->db->where('user_id', $id);
		$this->db->where('MONTH(tanggal_jual)', $bulan);
		$this->db->group_by('MONTH(tanggal_jual)'); // Menambahkan kolom bulan dalam klausa GROUP BY
		$query = $this->db->get();
	...
	public function get_pembelian($id, $bulan, $tahun)
{
	$query = "SELECT SUM(total_beli) as total_beli, SUM(unit) as unit, tanggal_beli, produk_name 
	FROM pembelian  
	WHERE user_id = $id AND MONTH(tanggal_beli) = $bulan AND YEAR(tanggal_beli) = $tahun 
	GROUP BY produk_name, DAY(tanggal_beli), tanggal_beli";
	return $this->db->query($query)->result_array();
}
    ...
    // }
	public function get_penjualan($id, $bulan, $tahun) {
        $this->db->select('SUM(total_untung) as total_untung, SUM(unit) as unit, MIN(tanggal_jual) as tanggal_jual, produk_name, COUNT(produk_name) as input');
        $this->db->from('penjualan');
        $this->db->where('user_id', $id);
        $this->db->where('MONTH(tanggal_jual)', $bulan);
        $this->db->where('YEAR(tanggal_jual)', $tahun);
        $this->db->group_by('produk_name, DAY(tanggal_jual)');
        $query = $this->db->get();
